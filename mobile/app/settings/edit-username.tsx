...
export default function EditUsername() {
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    try {
      const response = await axios.patch(`${API_URL}/api/settings/profile/`, { username })
      onUpdateUser(response.data)
      router.back()
    } catch (e: any) {
      setError(e.response?.data?.username ?? 'Something went wrong')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <View className="flex-1 bg-white">
      <View className="flex-row items-center px-6 pt-14 pb-4">
        ...
        <Text className="text-2xl font-black text-gray-900 tracking-tight ml-2">Username</Text>
      </View>
      ...
      <ConfirmModal
        ...
        message={`Change your username to "${username}"?`}
        confirmLabel="Save"
